...
<div id="ticket-create" class="w-full bg-white rounded-lg p-6 space-y-6">
            ...
            <div class="col-span-2">
                ...
                <div class="flex flex-wrap gap-2 mt-1">
                    <?php foreach ($attachment_list as $attachment): ?>
                        <?php if ($attachment->getTicketId() == $selected_ticket->getTicketId()): ?>
                            <div class="flex items-center gap-2">
                                <a class="text-blue-500 underline"
                                   href="index.php?route=attachment/preview&file=<?= urlencode($attachment->getFilePath()) ?>"
                                   target="_blank">
                                    <?= htmlspecialchars($attachment->getFileName()) ?>
                                </a>
                                <?php if ($canEdit): ?>
                                    <form method="GET" action="index.php" onsubmit="return confirm('Are you sure you want to delete this attachment?');" style="display:inline;">
                                        <input type="hidden" name="route" value="attachment/delete">
                                        <input type="hidden" name="attachment_id" value="<?= $attachment->getAttachmentId() ?>">
                                        <input type="hidden" name="ticket_id" value="<?= $selected_ticket->getTicketId() ?>">
                                        <button type="submit" class="text-red-500 hover:underline" title="Usuń załącznik">Usuń</button>
                                    </form>
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
    ...
    <?php if ($canEdit): ?>
        ...
        <!-- Edit Form -->
        <form id="edit_form" class="space-y-4 hidden" method="POST" action="index.php?route=ticket/edit"
    ...
    <div class="bg-gray-100 rounded p-4">
        ...
        <form action="index.php?route=comment/add" method="POST" class="mt-4 space-y-2">
            <input type="hidden" name="ticket_id" value="<?= htmlspecialchars($selected_ticket->getTicketId()) ?>">
        ...
        <?php if ($_SESSION['role_id'] != 4): ?>
            <form action="index.php?route=ticket/delete"></form>
        <?php endif; ?>
    </div>
</div>
    ...
    <div id="deletePopup" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 hidden">
        <div class="bg-white p-6 rounded-xl shadow-lg max-w-sm w-full text-center">
            ...
            <div class="flex justify-center gap-4">
                <form method="POST" action="index.php?route=ticket/delete">
